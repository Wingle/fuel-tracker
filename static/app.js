...
/** Wrapper around fetch that auto-adds auth header and handles 401 */
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        if (!res.ok) return; // apiFetch handles 401 redirect
...
function getVehicleParam() { return `vehicle_id=${currentVehicleId}`; }

// ---------------------------------------------------------------------------
// Vehicle modal
// ---------------------------------------------------------------------------
function openVehicleModal() { renderVehicleList(); resetVehicleForm(); document.getElementById("vehicleOverlay").classList.add("active"); }
function closeVehicleModal(e) { if (e && e.target !== e.currentTarget) return; document.getElementById("vehicleOverlay").classList.remove("active"); loadVehicles(); }

function renderVehicleList() {
    const div = document.getElementById("vehicleList");
    if (vehicles.length === 0) { div.innerHTML = '<p class="text-muted" style="text-align:center; padding:12px;">还没有车辆，请先添加</p>'; return; }
    div.innerHTML = `<div class="vehicle-items">${vehicles.map(v => `
        <div class="vehicle-item">
            <div class="vehicle-info"><strong>${v.name}</strong>${v.plate_number ? `<span class="text-muted">${v.plate_number}</span>` : ''}</div>
...
async function saveVehicle() {
    ...
    try {
        ...
        const res = await apiFetch(url, { method, headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, plate_number: plate }) });
        if (!res.ok) { const e = await res.json(); throw new Error(e.detail); }
        const r = await apiFetch(`${API}/vehicles`);
        vehicles = await r.json();
        renderVehicleList(); renderVehicleSelect(); resetVehicleForm();
...
async function deleteVehicle(id) {
    ...
    try {
        ...
        const r = await apiFetch(`${API}/vehicles`);
        vehicles = await r.json();
        renderVehicleList(); renderVehicleSelect();
...
async function loadRecords() {
    try {
        const res = await apiFetch(`${API}/records?${getVehicleParam()}&page=${currentPage}&page_size=${pageSize}`);
        const data = await res.json();
        records = data.items; totalRecords = data.total; totalPages = data.total_pages; currentPage = data.page;
...
async function loadStats() {
    try {
        const res = await apiFetch(`${API}/stats?${getVehicleParam()}`);
...
function renderTable() {
    ...
    tbody.innerHTML = records.map(r => `<tr>
        <td>${r.date}</td><td>${r.fuel_type || '<span class="text-muted">-</span>'}</td><td>${formatNum(r.mileage)}</td>
...
function renderPagination() {
    ...
    for (const p of getPageRange(currentPage, totalPages, 5)) {
        if (p === "...") { const s = document.createElement("span"); s.className = "btn-page"; s.style.border = "none"; s.style.cursor = "default"; s.textContent = "..."; nums.appendChild(s); }
        else { const b = document.createElement("button"); b.className = "btn btn-page" + (p === currentPage ? " active" : ""); b.textContent = p; b.onclick = () => goToPage(p); nums.appendChild(b); }
    }
}
function getPageRange(c, t, m) { if (t <= m) return Array.from({length:t},(_,i)=>i+1); const p=[],h=Math.floor(m/2); let s=Math.max(2,c-h),e=Math.min(t-1,c+h); if(c-h<2)e=Math.min(t-1,m-1); if(c+h>t-1)s=Math.max(2,t-m+2); p.push(1); if(s>2)p.push("..."); for(let i=s;i<=e;i++)p.push(i); if(e<t-1)p.push("..."); if(t>1)p.push(t); return p; }
function goToPage(page) { if (page < 1 || page > totalPages || page === currentPage) return; currentPage = page; loadRecords(); }
function onPageSizeChange() { pageSize = parseInt(document.getElementById("pageSizeSelect").value); localStorage.setItem("fuel_page_size", pageSize); currentPage = 1; loadRecords(); }

// ---------------------------------------------------------------------------
// Smart calculation (unchanged)
// ---------------------------------------------------------------------------
function onFuelInput(field) {
    if (lastEdited.length === 0 || lastEdited[lastEdited.length - 1] !== field) { lastEdited.push(field); if (lastEdited.length > 2) lastEdited.shift(); }
    ...
    if (edited.has("volume") && edited.has("unit_price") && v > 0 && u > 0) { document.getElementById("fTotalPrice").value = (v * u).toFixed(2); document.getElementById("fTotalPrice").classList.add("auto-calculated"); }
    else if (edited.has("volume") && edited.has("total_price") && v > 0 && t > 0) { document.getElementById("fUnitPrice").value = (t / v).toFixed(2); document.getElementById("fUnitPrice").classList.add("auto-calculated"); }
    else if (edited.has("unit_price") && edited.has("total_price") && u > 0 && t > 0) { document.getElementById("fVolume").value = (t / u).toFixed(2); document.getElementById("fVolume").classList.add("auto-calculated"); }
}
...
function openAddModal() {
    ...
    document.getElementById("editId").value = ""; document.getElementById("modalTitle").textContent = "添加加油记录";
    document.getElementById("recordForm").reset(); document.getElementById("fDate").value = todayStr();
    document.getElementById("fFuelType").value = "92#";
    lastEdited = []; ["fVolume","fUnitPrice","fTotalPrice"].forEach(id => document.getElementById(id).classList.remove("auto-calculated"));
    document.getElementById("modalOverlay").classList.add("active");
}
function openEditModal(id) {
    const rec = records.find(r => r.id === id); if (!rec) return;
    document.getElementById("editId").value = id; document.getElementById("modalTitle").textContent = "编辑加油记录";
    document.getElementById("fDate").value = rec.date; document.getElementById("fMileage").value = rec.mileage;
    document.getElementById("fFuelType").value = rec.fuel_type || "92#";
    document.getElementById("fVolume").value = rec.volume ?? ""; document.getElementById("fUnitPrice").value = rec.unit_price ?? "";
    document.getElementById("fTotalPrice").value = rec.total_price ?? ""; document.getElementById("fNote").value = rec.note ?? "";
    lastEdited = []; ["fVolume","fUnitPrice","fTotalPrice"].forEach(id => document.getElementById(id).classList.remove("auto-calculated"));
    document.getElementById("modalOverlay").classList.add("active");
}
...
async function confirmDelete() {
    ...
    try { await apiFetch(`${API}/records/${deleteTargetId}`, { method: "DELETE" }); closeDeleteModal(); await loadData(); }
    catch (e) { alert("删除失败: " + e.message); }
}
...
async function handleSubmit(e) {
    ...
    const payload = { vehicle_id: currentVehicleId, date: document.getElementById("fDate").value,
    ...
    try {
        ...
        const res = await apiFetch(url, { method: editId ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
        if (!res.ok) { const err = await res.json(); throw new Error(err.detail || "保存失败"); }
        closeModal(); if (!editId) currentPage = 1; await loadData();
    } catch (e) { alert("保存失败: " + e.message); }
}
...
function exportCSV() {
    if (!currentVehicleId) return;
    // For CSV export we need to pass token via query param since it opens a new tab
    window.open(`${API}/export/csv?${getVehicleParam()}&token=${getToken()}`, "_blank");
}

function openImportModal() {
    if (!currentVehicleId) { alert("请先添加一辆车"); return; }
    clearFile(); document.getElementById("importResult").style.display = "none"; document.getElementById("importResult").innerHTML = "";
...
function onFileSelected() { const input = document.getElementById("importFile"); if (input.files.length > 0) { document.getElementById("filePrompt").style.display = "none"; document.getElementById("fileSelected").style.display = "flex"; document.getElementById("fileName").textContent = input.files[0].name; document.getElementById("importBtn").disabled = false; } }
function clearFile() { document.getElementById("importFile").value = ""; document.getElementById("filePrompt").style.display = "flex"; document.getElementById("fileSelected").style.display = "none"; document.getElementById("importBtn").disabled = true; }
...
async function doImport() {
    ...
    const btn = document.getElementById("importBtn"); btn.disabled = true; btn.textContent = "导入中...";
    ...
    try {
        ...
        let html = data.imported > 0
            ? `<div class="import-result success">✅ 成功导入 <strong>${data.imported}</strong> 条记录${data.skipped > 0 ? `，跳过 ${data.skipped} 条重复记录` : ''}</div>`
            : `<div class="import-result success">没有新记录需要导入${data.skipped > 0 ? `（${data.skipped} 条已存在）` : ''}</div>`;
        if (data.errors && data.errors.length > 0) html += `<div class="import-result error" style="margin-top:8px;">⚠️ ${data.errors.length} 个错误:<div class="error-list">${data.errors.join("<br>")}</div></div>`;
        resultDiv.innerHTML = html; resultDiv.style.display = "block";
        if (data.imported > 0) { currentPage = 1; await loadData(); }
    } catch (e) { resultDiv.innerHTML = `<div class="import-result error">❌ ${e.message}</div>`; resultDiv.style.display = "block"; }
